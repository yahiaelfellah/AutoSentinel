...
while(i_file != "end"):
    ...
    avg_temp = np.zeros([160,160],np.uint) # bon j'ai pas trouver la deffirence entre le uint8 et le uint , mais finalement sa sere Ã  definir une plage d'entier 
    if( initialisation <= int(N)):
    ...
    def dist(cur_cent, last_cent):
        dis=(cur_cent[0]-last_cent[0])**2 + (cur_cent[1] - last_cent[1])**2 # calculer la somme de la variation au carrÃ© des deux pixel 
        return dis
    cluster_centres_q = np.zeros(shape=(0,2),dtype=np.int64) # on vas travaillÃ© sur un cluster de deux pixels
    ret, pure_img = cap.read() # on extraire le 'cout'Ã©me frame du video qui sera enregistrer dans la variable pure_img , ret , est un variable booleanretourner par la fonction read  
    while(cap.isOpened() and ret==True): # tanque la video qu'on souhaite traiter est en boucle 
            #time.sleep(0.1)
            count[i-1] = count[i-1] + 1 # on va commencer Ã  traiter les frame , du coup il faux incrÃ©menter le count[i-1] Ã  chaque fois qu'on est entrain de traiter une frame ( count[i-1] represente le nbr des frame traiter )
            #print(count[i-1])
            #print(nframe)
            img = cv2.resize(pure_img,(160,160)) # grase Ã  resize on arrive Ã  resizer notre image sans perdre la forme generale de l'image , c'est dans le sense ou on est pas entrais deretrancher les pixels d'une facon stupide bon maby il vas engondrer une certaine distortion l'orsqu'on zoom , euuh mais bon :p
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # convertir l'image en une image noire et blanch 
    
            colq[i-1].enqueue(gray) # aywah ahiya :D , tana na7chiw il image ' des annÃ©es 60 :p ' fil pila mte3na 
            if(count[i-1] < nframe):
                    ...
                    sumq[i-1] = sumq[i-1] + gray - temp # yzid ce qui a changÃ© entre le frame du background wil frame ali 9e3id yikhdim fih
...
#           cv2.imshow("avg",avg_show)
    ...
    print('j ai terminÃ© ce video et je suis dans l attente du prochain video')
    while(os.stat('input.txt').st_size == 0):
        time.sleep(1)
    input_file=open('input.txt', 'r')
    i_file=input_file.read()
